...
test("429 and 503 retry before output while 400 does not retry", async () => {
  ...
  for (const scenario of scenarios) {
    ...
    try {
      ...
      assert.equal(mock.requests.length, scenario.retry ? 2 : 1);
      const terminal = events.at(-1);
      if (scenario.retry) {
        ...
        if (usage?.type === "usage") {
          assert.equal(usage.usage.retryCount, 1);
          assert.equal(
            usage.usage.providerRequestId,
...
test("connection failures retry only before the first visible delta", async () => {
  ...
  for (const scenario of preVisibleScenarios) {
    ...
    try {
      ...
      assert.equal(events.at(-1)?.type, "completed", scenario);
      assert.equal(mock.requests.length, 2, scenario);
      const usage = events.find((event) => event.type === "usage");
      assert.equal(usage?.type, "usage");
      if (usage?.type === "usage") assert.equal(usage.usage.retryCount, 1);
    } finally {
      await mock.close();
    }
  }
  ...
  for (const status of [401, 403] as const) {
    ...
    try {
      ...
      if (terminal?.type === "failed") {
        assert.equal(terminal.code, "provider_error");
        assert.equal(terminal.retryable, false);
        assert.equal(terminal.providerRequestId, `auth-${status}`);
      }
    } finally {
